...
void SI4735::setFrequency(uint16_t freq)
{
  ...
  uint8_t actualAGCDIS;
  uint8_t actualAGCIDX;
  
    if (currentSsbStatus != 0)
    {
  this->getAutomaticGainControl();
  actualAGCDIS = currentAgcStatus.refined.AGCDIS;
  actualAGCIDX = currentAgcStatus.refined.AGCIDX;
...
if (currentSsbStatus != 0)
{
this->setAutomaticGainControl(actualAGCDIS, actualAGCIDX);
}
}
